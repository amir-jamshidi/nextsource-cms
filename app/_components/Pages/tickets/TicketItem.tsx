    ...
    return (
        <div className='ticket-list h-16 gap-2 px-4'>
            ...
            <div className='col-span-2 h-full w-full flex flex-col justify-center '>
                ...
                <p className='font-ir-medium text-primary-600 text-xs tracking-tight'>{user.email}</p>
            </div>
            <div className='col-span-2 h-full w-full flex items-center'>
                <p className='font-ir-medium text-sm text-primary-800 tracking-tight'>{ticket.code}</p>

            </div>
            <div className='col-span-1 h-full w-full flex justify-center flex-col'>
                <p className='font-ir-medium text-sm text-primary-800 tracking-tight'>{new Date(ticket.createdAt || 0).toLocaleDateString('fa-IR')}</p>
                <p className='font-ir-medium text-primary-600 text-xs tracking-tight'>{new Date(ticket.createdAt || 0).toLocaleTimeString('fa-IR')}</p>
            </div>
            <div className='col-span-1 h-full w-full flex items-center justify-center'>
                <span className='text-xs bg-green-200 rounded-xl px-2 py-1 font-ir-bold text-primary-800 tracking-tight'>{section.title}</span>
            </div>

            <div className='col-span-1 h-full w-full flex items-center justify-center'>
                {!ticket.isAnswer && <span className='text-xs bg-rose-200 rounded-xl px-2 py-1 font-ir-bold text-primary-800 tracking-tight'>بدون پاسخ</span>}
                {ticket.isAnswer && <span className='text-xs bg-green-200 rounded-xl px-2 py-1 font-ir-bold text-primary-800 tracking-tight'>با پاسخ</span>}
            </div>
