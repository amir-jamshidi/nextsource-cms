    ...
    return (
        <div className='ticket-list h-16 gap-2 px-4'>
            ...
            <div className='col-span-1 h-full w-full flex flex-col justify-center '>
                <p className='font-ir-bold text-sm text-primary-800 tracking-tight'>{alert.title}</p>
            </div>
            <div className='col-span-4 h-full w-full flex items-center'>
                <p className='font-ir-medium text-sm text-primary-800 tracking-tight'>{alert.body}</p>
            </div>
            <div className='col-span-1 h-full w-full flex justify-center flex-col'>
                {alert.href && (
                    <p className='font-ir-medium text-sm text-primary-800 tracking-tight'>{alert.href}</p>
                )}
                {!alert.href && (
                    <p className='font-ir-medium text-sm text-primary-800 tracking-tight'>ندارد</p>
                )}
            </div>
            <div className='col-span-1 h-full w-full flex justify-center items-center'>
                {alert.type === 'SUCCESS' && <span className='text-xs bg-green-200 rounded-xl px-2 py-1 font-ir-bold text-primary-800 tracking-tight'>موفق</span>}
                {alert.type === 'WARNING' && <span className='text-xs bg-amber-200 rounded-xl px-2 py-1 font-ir-bold text-primary-800 tracking-tight'>هشدار</span>}
                {alert.type === 'ERROR' && <span className='text-xs bg-rose-200 rounded-xl px-2 py-1 font-ir-bold text-primary-800 tracking-tight'>خطا</span>}
            </div>
