    ...
    return (
        <div className="h-14 bg-white dark:bg-primary-900 rounded-xl flex items-center px-4 gap-x-2 shadow-sm border border-transparent dark:border-primary-800/50">
            {option.options.map(item => (
                <button onClick={() => handleClick(item.value)} key={item.value} className={`${currentDay == item.value ? 'bg-blue-400 dark:bg-blue-600 text-primary-0 ' : 'bg-primary-0 dark:bg-primary-800 text-primary-700 dark:text-primary-50'} px-4 rounded-xl py-1.5 dark:hover:bg-blue-700 hover:bg-blue-500 transition-all hover:text-primary-0 cursor-pointer`}>{item.title}</button>
            ))}
        </div>
    )
}
